...
| `Search` | `DocumentSearch` (hybrid with per-clause project filter; degrades to BM25 if embedding fails — never throw the search away), `DocumentCatalog` (list/get/similar reads for the Phase 8 MCP tools), `StatusService` (Phase 10 per-project rollups), `ProjectDescriber` (the one-project detail view behind `rtfm describe project` — status + catalog + notes + contradiction rollup + the on-disk state stores, assembled, no new fields) |
